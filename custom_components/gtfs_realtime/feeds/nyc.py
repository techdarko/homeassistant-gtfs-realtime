...
STATIC_SUPPLEMENTAL = "http://web.mta.info/developers/files/google_transit_supplemented.zip"
...
def subway(route_ids: Iterable[str], stop_ids: Iterable[str], api_key: str = "", route_icons: os.PathLike | None = None) -> str:
    ...
    gtfs_config = {
        DOMAIN: {
            API_KEY: api_key,
            URL_ENDPOINTS: [x for x in {FEEDS.get(route_id) for route_id in route_ids} if x is not None],
